...
static struct GPU __GPU __attribute__((section(".KERNEL_DATA"))); // Se declara esta variable global interna la cual simplifica el codigo
...
void __attribute__((section(".KERNEL_CODE"))) GPU_TFT_manufacturerCMD(){
...
void __attribute__((section(".KERNEL_CODE"))) GPU_TFT_init(){
    SPI_TFT_CS_ENABLE;
    SPI_TFT_DC_ENABLE;
    SPI_TFT_DESELECT;
...
void __attribute__((section(".KERNEL_CODE"))) GPU_drawablesRender(TableDrawables *table, struct Tile* tiles){
...
void __attribute__((section(".KERNEL_CODE"))) GPU_new(){
